...
print(selection_method(nba_leader_list))


print('Insertion.')
def insertion_counter(list):
    for pos in range(1, len(list)):
        key_pos = pos
        scan_pos = key_pos - 1
        key_value = list[key_pos][1]
        while list[scan_pos][1] > key_value and scan_pos >= 0:
            list[scan_pos + 1][0] = list[scan_pos][0]
            scan_pos -= 1
        list[scan_pos + 1][1] = key_value
    return list

print(insertion_counter(nba_leader_list))